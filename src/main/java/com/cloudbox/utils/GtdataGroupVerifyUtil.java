...
import com.cloudbox.mgmt.thread.RefreshGtdataGroupSignThread;
 ...
 * 
 */
public class GtdataGroupVerifyUtil {
    ...
    static {
        try {
            ...
            groupUserHost = StringTools.getValueFromProperties(file, "groupUserHost");
            commonUserHost = StringTools.getValueFromProperties(file, "commonUserHost");
            gtdataOutHost = StringTools.getValueFromProperties(file, "gtdataOutHost");
            user = StringTools.getValueFromProperties(file, "groupUserName");
            password = StringTools.getValueFromProperties(file, "groupUserPassword");
            userSize= StringTools.getValueFromProperties(file, "userSize");
            
            updateGroupUserGetSignAndTime();//先登录获取sign和time
            //启动定时更新线程
            Timer timer = new Timer();
            timer.schedule(new RefreshGtdataGroupSignThread(), 100,1000*60*10);//10分钟刷新一次
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
//		“http://<HOST>:<PORT>/?op=LOGIN[&user=<user name>][&time=<time>][&token=<token>][&pass=<password>]"
    ...
    public static void main(String[] args)
	{
    	...
    	try {
    		Map<String, Object> map =GtdataFileUtil.createNewDirectory("DatamanagementSys", "temp");
    	...
    	System.out.println(groupUserRegisterCommonUser("DatamanagementSys", PubFun.MD5("li556600"), "5000000000", GtdataGroupVerifyUtil.getSign(), GtdataGroupVerifyUtil.getSignTime()));
    	...
    	//System.out.println(GtdataGroupVerifyUtil.getSign()+","+GtdataGroupVerifyUtil.getSignTime());
	}
