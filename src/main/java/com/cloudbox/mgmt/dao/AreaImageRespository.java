...
@Service
public class AreaImageRespository extends JdbcRepository<AreaImage, String> {
	@Autowired
	...
	public List queryproductInformationByGeom(String geom, String table,
			...
			String productId, String ProductsourceTypeCode,
		...
		if (geom != null) {
			wheresql += "ST_Intersects(gemo,ST_GeomFr"
					+ "omText('" + geom + "'))"
					+ " and ";
		}
		if (ProductsourceTypeCode != null) {
			wheresql += "image_product_type='" + ProductsourceTypeCode
					+ "' and ";
		...
		if (Timerange != null) {
			...
			/*
			 * wheresql += "collect_time between '" + Timerangearray[0] +
			 * "' and '" +Timerangearray[1] + "' and ";
			 */
			wheresql += "collect_start_time >='" + Timerangearray[0]
					+ "' and collect_start_time <= '" + Timerangearray[1]
					+ " 23:59:59' and ";
		}
		if (resolutionrange != null) {
			String[] resolutionrangearray = resolutionrange.split(",");
			wheresql += "image_start_resolution >= '" + resolutionrangearray[0]
	...
	public List queryproductInformationByAreaId(String provinceId,
			...
			String cloudsrange, String Timerange, String productId,
			String ProductsourceTypeCode, String resolutionrange) {
		String sql = "select id, area_no, image_row_col, image_satellite_type, image_start_resolution,image_end_resolution,"
				+ "image_spectrum_type, begin_time, update_time, ST_AsText(gemo) as gemo, range, num, "
				+ "is_cover, image_area, data_id, image_product_type, relation_no, "
				+ " name, image_cloudage,sensor_id,product_level,to_date(collect_start_time,'yyyy/mm/dd') as collect_start_time,to_date(collect_end_time,'yyyy/mm/dd') as collect_end_time,product_id,file_path,area_no_array ,img_url,area_description from "
	...
	public List<RsImageMetaData> queryproductInformationByRecordid(
			String recordid) {
		String sql = "select id, area_no, image_row_col, image_satellite_type, image_start_resolution,image_end_resolution,"
				+ "image_spectrum_type, begin_time, update_time, ST_AsText(gemo) as gemo, range, num, "
				+ "is_cover, image_area, data_id, image_product_type, relation_no, "
	...
	public Map<String, Object> queryproductInformationById(String id) {
		...
		if (list.size() > 0) {
			return (Map<String, Object>) dao.queryForList(sql, "jdbctemplate")
					.get(0);
		}
		return null;
	}
	...
	public String queryRecordIdByName(String name) {
		String sql = "select record_id from "
				+ Constant.rsclouds_rscm_area_image_table + " where name='"
				+ name + "'";
